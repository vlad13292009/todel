...
class QuizSession(models.Model):
    quiz = models.ForeignKey(
        Quiz, on_delete=models.CASCADE, related_name="sessions", verbose_name="Квиз"
    )
    unique_code = models.CharField(
        max_length=10, unique=True, verbose_name="Уникальный код для входа"
    )
    password = models.CharField(max_length=50, blank=True, verbose_name="Пароль сессии")
...
class ParticipantSession(models.Model):
    ...
    participant_name = models.CharField(
        max_length=100, blank=True, verbose_name="Имя участника"
    )
